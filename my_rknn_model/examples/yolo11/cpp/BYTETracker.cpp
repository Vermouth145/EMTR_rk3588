...
BYTETracker::BYTETracker(int frame_rate, int track_buffer)   //Ĭ��ֵΪ30��90
{
	track_thresh = 0.5;
	high_thresh = 0.6;
	match_thresh = 0.8;
	...
	frame_id = 0;
	max_time_lost = track_buffer;  //��󻺴�֡��
...
vector<STrack> BYTETracker::update(const vector<Object>& objects)
{
	...
	if (!objects.empty())
	{
		for (int i = 0; i < (int)objects.size(); i++)
		{
			...
			// tlbr_[0] = objects[i].bbox.x1;
			// tlbr_[0] = objects[i].bbox.y1;
			// tlbr_[0] = objects[i].bbox.x2;
			// tlbr_[0] = objects[i].bbox.y2;

			// std::cout << "X1 " << objects[i].bbox.x1 << std::endl;
			// std::cout << "Y1 " << objects[i].bbox.y1 << std::endl;
			// std::cout << "X2 " << objects[i].bbox.x2 << std::endl;
			// std::cout << "Y2 " << objects[i].bbox.y2 << std::endl;

			float score = objects[i].prob;


			STrack strack(STrack::tlbr_to_tlwh(tlbr_), score);   //tlbr_to_tlwh:���ĵ������ʾ��Ϊ����+����
	...
	// Prune removed_stracks to avoid unbounded memory growth
	if (this->removed_stracks.size() > 1000)
	{
		this->removed_stracks.erase(this->removed_stracks.begin(),
		                            this->removed_stracks.begin() + (int)this->removed_stracks.size() - 500);
	}
	...
	output_stracks.reserve(this->tracked_stracks.size());
	for (int i = 0; i < (int)this->tracked_stracks.size(); i++)
	{
		if (this->tracked_stracks[i].is_activated)
		{
