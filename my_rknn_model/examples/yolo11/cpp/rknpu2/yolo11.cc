...
int init_yolo11_model(const char *model_path, rknn_app_context_t *app_ctx)
{
    ...
    if (ret < 0)
    {
        ...
        // 2. 设置NPU核心掩码（关键位置！）
//    rknn_core_mask core_mask = RKNN_NPU_CORE_0_1_2; // 使用3个NPU核心
//    ret = rknn_set_core_mask(app_ctx->rknn_ctx, core_mask);
//    if (ret != RKNN_SUCC) {
//        printf("WARNING: set core_mask failed!\n");
//        // 可以继续运行，但性能可能受影响
//    }
...
{
    double preprocess_time = 0, npu_time = 0, postprocess_time = 0;
    TimeCounter preprocess_timer, npu_timer, postprocess_timer;
    int ret;
    ...
    npu_time = npu_timer.stop();

    // Post Process
    postprocess_timer.start();
    post_process(app_ctx, outputs, &letter_box, box_conf_threshold, nms_threshold, od_results);
    postprocess_time = postprocess_timer.stop();

    printf("[PERF] Preprocess: %.2f ms | NPU: %.2f ms | Postprocess: %.2f ms\n",
       preprocess_time, npu_time, postprocess_time);
