...
//    inputs[i].size = width * height * channel *4;
    inputs[0].size = width * height * channel *3;
...
int rknn_lite::interf()
{
    ...
    cv::Mat imgs[3];
    cv::cvtColor(ori_img, imgs[0], cv::COLOR_BGR2RGB);
    cv::cvtColor(ori_img2, imgs[1], cv::COLOR_BGR2RGB);
    cv::cvtColor(ori_img3, imgs[2], cv::COLOR_BGR2RGB);
//    cv::cvtColor(ori_img4, imgs[3], cv::COLOR_BGR2RGB);
    int img_width = imgs[0].cols;
    int img_height = imgs[0].rows;

    letter_box.in_height = img_height;
    letter_box.in_width = img_width;
    letter_box.channel = imgs[0].channels();
    letter_box.target_width = width;
    letter_box.target_height = height;
    compute_letter_box(&letter_box);  // 只需计算一次
    ...
    in_data_batch = (unsigned char *)malloc(3 * img_size);
//    in_data_batch = (unsigned char *)malloc(1 * img_size);
    if (!in_data_batch)
    {
        return -1;
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (img_width != width || img_height != height) {
            if (imgs[i].rows != letter_box.resize_height ||
                imgs[i].cols != letter_box.resize_width) {
                // 先缩放
                cv::resize(imgs[i], processed_img,
                ...
                // 直接填充
                cv::copyMakeBorder(imgs[i], processed_img,
                    letter_box.h_pad_top, letter_box.h_pad_bottom,
                    letter_box.w_pad_left, letter_box.w_pad_right,
                    cv::BORDER_CONSTANT, cv::Scalar(114, 114, 114));
            }
            ...
            // 尺寸相同时直接使用原图
            processed_img = imgs[i];
        }

        // 关键步骤：将处理后的图像数据拷贝到in_data_batch
        memcpy(in_data_ptr, processed_img.data, img_size);
        in_data_ptr += img_size;  // 指针移动到下一张图位置
    }
...
//                 box_conf_threshold, nms_threshold, scale_w, scale_h, out_zps, out_scales, &detect_result_group);
    int a = post_process(rkModel, outputs, &letter_box, box_conf_threshold, nms_threshold, &detect_result_group, width, height, io_num, output_attrs, true, 3);
...
//        printf("Detect result camera id:%d...\n", det_result->cameraid);
//        sprintf(text, "%d %.1f%%", det_result->class_index, det_result->prop * 100);
      ...
      sprintf(text, "%d %.1f%%", det_result->class_index, det_result->prop * 100);
//      printf("%d @ (%d %d %d %d) %f\n", det_result->class_index, det_result->box.left, det_result->box.top,
//            det_result->box.right, det_result->box.bottom, det_result->prop);
      switch (det_result->cameraid)
      {
      case 1:
        rectangle(ori_img, cv::Point(x1, y1), cv::Point(det_result->box.right, det_result->box.bottom), cv::Scalar(0, 0, 255), 3);
        putText(ori_img, text, cv::Point(x1, y1 + 12), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255));
        break;
      case 2:
        rectangle(ori_img2, cv::Point(x1, y1), cv::Point(det_result->box.right, det_result->box.bottom), cv::Scalar(0, 0, 255), 3);
        putText(ori_img2, text, cv::Point(x1, y1 + 12), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255));
        break;
      case 3:
        rectangle(ori_img3, cv::Point(x1, y1), cv::Point(det_result->box.right, det_result->box.bottom), cv::Scalar(0, 0, 255), 3);
        putText(ori_img3, text, cv::Point(x1, y1 + 12), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255));
        break;
//      case 4:
//        rectangle(ori_img4, cv::Point(x1, y1), cv::Point(det_result->box.right, det_result->box.bottom), cv::Scalar(0, 0, 255), 3);
