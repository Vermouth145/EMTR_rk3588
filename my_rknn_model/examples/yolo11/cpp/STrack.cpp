...
void STrack::multi_predict(vector<STrack*> &stracks, byte_kalman::KalmanFilter &kalman_filter)
{
	...
	{
		if (stracks[i]->state != TrackState::Tracked)   //stateÓÐ TrackState.New£¬Tracked, Lost, RemovedËÄÖÖ×´Ì¬
		{
			stracks[i]->mean[7] = 0;
		}
		kalman_filter.predict(stracks[i]->mean, stracks[i]->covariance);   //Ô¤²â¿¨¶ûÂüÂË²¨Æ÷µÄÏÂÒ»¸ö×´Ì¬
